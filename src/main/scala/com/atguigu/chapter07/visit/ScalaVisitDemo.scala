...
object Testvisit {
  def main(args: Array[String]): Unit = {
    ...
    //println(c.job) // 从编译器层面，protected 属性，只能在子类，本类，或者 伴生对象访问
...
//3. 方法的修饰符，默认是public
class Clerk {
  var name: String = "jack" // => public name public name_$eq
  protected var job : String = "大数据高级工程师" // public job() public job_$eeq
  //对 private[chapter07] 说明
  //1. private 仍然作用，但是扩大访问范围
  //2. 可以在com.atguigu.chapter07 和它子包下面访问 sal
  private[chapter07] var sal: Double = 9999.9 // private sal() private sal_$eq
...
object Clerk {
  def test(c: Clerk): Unit = {
    ...
    println("test() name=" + c.name + " sal= " + c.sal + "job=" + c.job)
    c.sayHi()
  }
}
